...
if st.button("데이터 조회 및 엑셀 생성"):
    ...
    if not target_companies:
        ...
        with st.spinner("KRX 코스피 및 코스닥 데이터를 모두 불러오는 중..."):
            try:
                # 선택한 날짜를 API 규격(YYYYMMDD)에 맞게 변환
                search_date_str = selected_date.strftime("%Y%m%d")
                
                # 유가증권(코스피) 및 코스닥 API 주소
                ...
                if not combined_data:
                    st.error(f"{selected_date} 일자의 데이터가 없습니다. (주말, 공휴일이거나 데이터 업데이트 전일 수 있습니다. 다른 날짜를 선택해보세요.)")
                else:
                    ...
                    if filtered_df.empty:
                        st.warning("입력하신 기업과 일치하는 종목이 코스피/코스닥 양쪽 시장 모두에 없습니다. 정확한 이름을 입력했는지 확인해주세요.")
                    else:
                        # 핵심단어(종목명, 종가, 시가총액) 추출 및 정리
